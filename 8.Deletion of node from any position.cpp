// 8. Deletion of node from any position of Singly Linked list
...
void deleteAtAny()
{
	...
	if (pos<=0 || (pos>=count && head==NULL))		//if your head is null it means your list is empty
	{												//let's say if you are trying to insert at 4th position but your head is empty.so it'll break the condition
		printf("Invalid operation \n");	
	}
	else if (pos==1 && head!=NULL)					//to delete first node of List
	{
		node *temp=head;
		head=head->next;							//will make the second node/NULL(for length=1) as the head of the List
		free(temp);									//to free memory of node to be deleted
		printf("Node deleted successfully \n");								
	}
	else											//to delete at any position from second including last
	{
		node *t=head,*prev;
		...
		{
			prev->next=t->next;						//previous node->next stores the node to be delted->next, thus the link of node to be deleted is removed from the List
			node *temp=t;							
			free(temp);								//to free memory of node to be deleted
			printf("Node deleted successfully \n");	
		}
...
int main()
{
	...
	do
	{
		...
		printf("2. to delete node from any position \n");
